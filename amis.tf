...
locals {
  ...
  all_ami_keys = "${sort(concat(
    local.win_all_ami_keys,
    local.lx_all_ami_keys,
    list(local.win_builder_ami_key),
    list(local.lx_builder_ami_key)
  ))}"
  ...
  ami_underlying = "${merge(
    ...
    map(local.lx_builder_ami_key, local.lx_builder_ami_key),
    map(local.win_builder_ami_key, local.win_all_ami_keys[0])
  )}"
...
locals {
  ...
  win_requests          = "${matchkeys(local.win_all_ami_keys, local.win_all_ami_keys, local.user_requests)}"
  lx_requests           = "${matchkeys(local.lx_all_ami_keys, local.lx_all_ami_keys, local.user_requests)}"
  ...
  win_pkg_requests      = "${matchkeys(local.win_pkg_ami_keys, local.win_pkg_ami_keys, local.user_requests)}"
  lx_pkg_requests       = "${matchkeys(local.lx_pkg_ami_keys, local.lx_pkg_ami_keys, local.user_requests)}"
  win_need_builder      = "${length(local.win_pkg_requests) > 0 ? 1 : 0}"
  lx_need_builder       = "${length(local.lx_pkg_requests) > 0 ? 1 : 0}"
  win_builder_list      = "${list(local.win_need_builder == 1 ? lookup(local.ami_underlying, local.win_builder_ami_key) : "")}"
  lx_builder_list       = "${list(local.lx_need_builder == 1 ? lookup(local.ami_underlying, local.lx_builder_ami_key) : "")}"
