...
resource "aws_instance" "spels" {
  ...
  count                        = "${length(matchkeys(values(data.null_data_source.spel_instance_amis.inputs),keys(data.null_data_source.spel_instance_amis.inputs),var.tfi_lx_instances))}"
  ami                          = "${lookup(matchkeys(values(data.null_data_source.spel_instance_amis.inputs),keys(data.null_data_source.spel_instance_amis.inputs),var.tfi_lx_instances), count.index)}"
