...
`TFI_WIN_INSTANCES` | [empty] | optional | Whether or not to build any/all Windows instances. Acceptable values are "win08", "win12", and/or "win16". If none are wanted, remove variable from CodeBuild.
`TFI_LX_INSTANCES` | [empty] | optional | Whether or not to build any/all Linux instances. Acceptable values are "centos6", "centos7", "rhel6" and/or "rhel7". If none are wanted, remove variable from CodeBuild.
